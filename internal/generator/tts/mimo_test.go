...
func TestMapMimoVoice(t *testing.T) {
	...
	if got := mapMimoVoice("default_zh"); got != "default_zh" {
		t.Fatalf("passthrough voice = %q", got)
	}
	if got := mapMimoVoice("zh-CN-XiaoxiaoNeural"); got != "default_zh" {
		t.Fatalf("zh voice = %q", got)
	}
	if got := mapMimoVoice("af_bella"); got != "default_en" {
		t.Fatalf("default voice = %q", got)
	}
}
